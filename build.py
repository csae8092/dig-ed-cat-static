...
for i, inst_objects in enumerate(inst_df.to_dict(orient="records")):
    ...
    item = {key: {"label": key, "data": value} for key, value in inst_objects.items()}
    previous_entry = None
    next_entry = None
    if i > 0:
...
for i, object in enumerate(df.to_dict(orient="records")):
    ...
    item = {key: {"label": labels[key], "data": value} for key, value in object.items()}
    for key, value in object.items():
        if labels[key] in M2M.keys():
            ...
            for x in value.split(";"):
                ...
                match_item = {
                    slugify(mkey).replace("-", "_"): {"label": mkey, "data": mvalue}
